...
class ConnectionCLI(BaseCLI):
        ...
        try:
            ...
            if db_type:
                ...
                if not db_type_obj:
                    ...
                    return 1
                query = query.filter_by(database_type_uuid=db_type_obj.uuid)
            ...
            for conn in connections:
                ...
                is_main = str(conn.uuid) == str(main_conn_id) if main_conn_id else False
        ...
        try:
            ...
            for db_type in db_types:
                ...
                conn_count = self.session.query(self.connection_model).filter_by(
                    database_type_uuid=db_type.uuid
                ).count()
    ...
    def show_connection(self, name, show_password=False):
        ...
        try:
            ...
            try:
                main_conn_id = self.db_manager.get_main_connection_id()
                is_main = str(conn.uuid) == str(main_conn_id)
            except:
                is_main = False
    ...
    def create_connection(self, name, db_type, connection_string, username=None, password=None, options=None):
        ...
        try:
            ...
            conn = self.connection_model(
                name=name,
                database_type_uuid=db_type_obj.uuid,
            ...
            self.output_success(f"Connection created: {name} ({conn.uuid})")
    ...
    def create_connection_interactive(self):
        ...
        try:
            ...
            conn = self.connection_model(
                name=name,
                database_type_uuid=selected_type.uuid,
                connection_string=connection_string,
                username=username,
                password=password,
            ...
            self.output_success(f"\nConnection created: {name} ({conn.uuid})")
    ...
    def update_connection(self, name, **kwargs):
        ...
        try:
            ...
            self.db_manager.invalidate_cache(conn.uuid)
    ...
    def delete_connection(self, name, force=False):
        ...
        try:
            ...
            try:
                main_conn_id = self.db_manager.get_main_connection_id()
                if str(conn.uuid) == str(main_conn_id):
    ...
    def _test_connection_internal(self, conn):
        ...
        try:
            ...
            engine = self.db_manager.get_engine(conn.uuid)
            ...
            if action == 'clear':
                ...
                if self.verbose and (engine_count > 0 or session_count > 0):
                    ...
                    for conn_id in self.db_manager.engines.keys():
                        ...
                        conn = self.session.query(self.connection_model).filter_by(uuid=conn_id).first()
                        if conn:
    ...
    def set_main_connection(self, name):
        ...
        try:
            ...
            self.output_info(f"Add to your config: MAIN_DB_CONNECTION_ID = '{conn.uuid}'")
