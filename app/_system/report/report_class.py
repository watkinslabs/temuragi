...
class ReportService:
    ...
    def create_report_template(self, name: str, display_name: str,
                             template_uuid: UUID, description: Optional[str] = None,
        ...
        try:
            self.session.commit()
            self.logger.info(f"Report template created: {name} ({report_template.uuid})")
    ...
    def get_report_template(self, template_id: Union[str, UUID]) -> Optional[ReportTemplate]:
        ...
        if isinstance(template_id, str) and not self._is_valid_uuid(template_id):
            # Try by name
            return self.session.query(ReportTemplate).filter_by(name=template_id).first()
        return self.session.query(ReportTemplate).filter_by(uuid=template_id).first()

    def update_report_template(self, template_uuid: UUID, **kwargs) -> ReportTemplate:
        """Update report template"""
        report_template = self.get_report_template(template_uuid)
        if not report_template:
            raise ValueError(f"Report template {template_uuid} not found")
    ...
    def delete_report_template(self, template_uuid: UUID, force: bool = False) -> bool:
        ...
        report_count = self.session.query(Report).filter_by(report_template_id=template_uuid).count()
    ...
    def create_report(self, slug: str, name: str, query: str, connection_id: UUID,
                     ...
                     report_template_id: Optional[UUID] = None, **kwargs) -> Report:
        ...
        connection = self.session.query(Connection).filter_by(uuid=connection_id).first()
        if not connection:
        ...
        try:
            ...
            self.logger.info(f"Report created: {slug} ({report.uuid})")
            return report
        except IntegrityError as e:
            self.session.rollback()
    ...
    def get_report(self, report_id: Union[str, UUID]) -> Optional[Report]:
        ...
        if isinstance(report_id, str) and not self._is_valid_uuid(report_id):
            # Try by slug
            return self.session.query(Report).filter_by(slug=report_id).first()
        return self.session.query(Report).filter_by(uuid=report_id).first()

    def update_report(self, report_uuid: UUID, **kwargs) -> Report:
        """Update report with validation"""
        report = self.get_report(report_uuid)
        if not report:
            raise ValueError(f"Report {report_uuid} not found")
    ...
    def delete_report(self, report_uuid: UUID) -> bool:
    ...
    def list_reports(self, category: Optional[str] = None,
                    ...
                    user_uuid: Optional[UUID] = None) -> List[Report]:
        ...
        # Filter by user permissions if user_uuid provided
        if user_uuid:
            return Report.get_user_reports(self.session, user_uuid)

        return reports

    def duplicate_report(self, report_uuid: UUID, new_name: str,
    ...
    def add_report_column(self, report_uuid: UUID, name: str, data_type_id: UUID,
        ...
        max_order = self.session.query(func.max(ReportColumn.order_index))\
                               .filter_by(report_id=report_uuid).scalar()
    ...
    def update_report_column(self, column_uuid: UUID, **kwargs) -> ReportColumn:
        """Update a report column"""
        column = self.session.query(ReportColumn).filter_by(uuid=column_uuid).first()
        if not column:
            raise ValueError(f"Report column {column_uuid} not found")
    ...
    def delete_report_column(self, column_uuid: UUID) -> bool:
    ...
    def reorder_columns(self, report_uuid: UUID, column_order: List[UUID]) -> bool:
        """Reorder report columns"""
        for index, column_uuid in enumerate(column_order):
            self.session.query(ReportColumn)\
                       .filter_by(uuid=column_uuid, report_id=report_uuid)\
    ...
    def add_report_variable(self, report_uuid: UUID, name: str,
                           variable_type_id: UUID, data_type_id: UUID,
    ...
    def update_report_variable(self, variable_uuid: UUID, **kwargs) -> ReportVariable:
        """Update a report variable"""
        variable = self.session.query(ReportVariable).filter_by(uuid=variable_uuid).first()
        if not variable:
            raise ValueError(f"Report variable {variable_uuid} not found")
    ...
    def delete_report_variable(self, variable_uuid: UUID) -> bool:
    ...
    def execute_report(self, report_uuid: UUID, request_data: Dict[str, Any],
                      user_uuid: Optional[UUID] = None) -> Dict[str, Any]:
        ...
        if user_uuid and not Report.check_permission(self.session, user_uuid, report.slug, 'execute'):
    ...
    def test_report_query(self, report_uuid: UUID, params: Optional[Dict] = None) -> List[Dict]:
    ...
    def get_report_execution_history(self, report_uuid: UUID,
                                   limit: int = 100) -> List[ReportExecution]:
        """Get execution history for a report"""
        return self.session.query(ReportExecution)\
                          .filter_by(report_id=report_uuid)\
    ...
    def assign_report_to_role(self, report_uuid: UUID, role_uuid: UUID,
        ...
        return report.assign_to_role(self.session, role_uuid, actions)

    def remove_report_from_role(self, report_uuid: UUID, role_uuid: UUID,
                               actions: Optional[List[str]] = None) -> Tuple[bool, str]:
        ...
        return report.remove_from_role(self.session, role_uuid, actions)

    def get_report_roles(self, report_uuid: UUID, action: str = 'view') -> List[Any]:
    ...
    def check_user_permission(self, user_uuid: UUID, report_slug: str,
                            action: str = 'view') -> bool:
        """Check if user has permission for report action"""
        return Report.check_permission(self.session, user_uuid, report_slug, action)
    ...
    def get_report_structure(self, report_uuid: UUID) -> Dict[str, Any]:
    ...
    def export_report_definition(self, report_uuid: UUID) -> Dict[str, Any]:
    ...
    def import_report_definition(self, definition: Dict[str, Any],
        ...
        data_types = {dt.name: dt.uuid for dt in self.list_data_types()}
        variable_types = {vt.name: vt.uuid for vt in self.list_variable_types()}
        ...
        for var_data in definition.get('variables', []):
            self.add_report_variable(
                ...
                data_type_id=data_types[var_data['data_type']],
    ...
    def analyze_report_columns(self, report_uuid: UUID, 
        ...
        Args:
            report_uuid: UUID of the report to analyze
    ...
    def sync_report_columns_from_query(self, report_uuid: UUID,
        ...
        for index, meta_col in enumerate(metadata_columns):
            ...
            if col_name in existing_by_name:
                ...
                # Check type change
                if existing_col.data_type_id != data_type.uuid:
                    updates['data_type_id'] = data_type.uuid
                ...
                if updates:
                    self.update_report_column(existing_col.uuid, **updates)
        ...
        if remove_missing:
            ...
            for existing_col in report.columns:
                if existing_col.name not in query_col_names:
                    self.delete_report_column(existing_col.uuid)
