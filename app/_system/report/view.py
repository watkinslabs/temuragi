...
def report_list():
    ...
    return renderer.render_page("report")
...
def view_report(report_id):
    ...
    try:
        ...
        report = g.session.query(Report).filter_by(uuid=report_id).first()
        ...
        if report.report_template:
            ...
            return renderer.render_template(
                template_uuid=report.report_template.template_id,
                context=context
            )
        else:
...
@bp.route('/edit/<report_id>', methods=['POST'])
def edit(report_id):
